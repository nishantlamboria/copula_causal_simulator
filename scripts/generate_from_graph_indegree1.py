...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate synthetic data from a DAG with max indegree <= 1."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Dataset config used to locate the coupling library.",
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="outputs/generated_indegree1",
...
def main() -> None:
    ...
    print(f"Loading coupling library for dataset: {dataset_id}")
    ...
    validate_dag(adjacency, max_allowed_indegree=1)

    generator = IndegreeOneGenerator(
    ...
    metadata = {
        ...
        "generator": "indegree_one_paircopula",
        "max_supported_indegree": 1,
        "u_file": str(u_path),
        "x_file": str(x_path),
