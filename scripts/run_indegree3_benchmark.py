...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run full indegree-3 benchmark: coupling library, conditional vines for indegrees two and three, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Force rebuilding conditional D-vine libraries for indegrees 2 and 3.",
    )
...
def main() -> None:
    ...
    conditional_vine_exists = (
        ...
        and (artifact_dir / "conditional_vine_library_indegree3.pkl").exists()
        and (artifact_dir / "conditional_vines_indegree3").exists()
    )
    ...
    # ------------------------------------------------------------------
    # 3. Generate random DAGs with max indegree 3
    # ------------------------------------------------------------------
    run_command([
        sys.executable,
    ...
    for graph_idx in range(args.num_graphs):
        ...
        generation_runtime_seconds = run_command([
            sys.executable,
            str(PROJECT_ROOT / "scripts" / "generate_from_graph_indegree3.py"),
            "--config",
            str(config_path),
            "--graph",
            str(graph_path),
